...
BOOL CConnectionMonitorWindow::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	if (fs) {
		...
		try {
			...
			ERROR_LOG << L"CConnectionMonitorWindow::OnInitDialog : settings.iniの読み込みに失敗";
			pt.clear();
		}
		CRect rcWindow;
...
void CConnectionMonitorWindow::OnDestroy()
{
	...
	try {
		...
		ERROR_LOG << L"CConnectionMonitorWindow::OnDestroy : settings.iniの読み込みに失敗";
		pt.clear();
	}
	...
	for (auto& conDataOperation : m_connectionDataOperationList) {
		...
		case CConnectionManager::kRemoveConnection:
		{
			...
			if (i != -1) {
				...
				SetTimer(conData->uniqueId, 1500);	// 遅延させてから消す
			}
		}
		break;
