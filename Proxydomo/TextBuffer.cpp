 ...
 *	@file	TextBuffer.cpp
 *	@brief
 *データを受け取ってテキストフィルターで処理してRequestManagerに返すクラス
 */
/**
...
CTextBuffer::CTextBuffer(CFilterOwner &owner, IDataReceptor *output)
    : m_owner(owner), m_output(output), m_bCharaCodeDectated(false),
      m_pConverter(nullptr), m_bDataDump(false), m_bJISCode(false),
      m_bBinary(false) {
#if 0
...
int CTextBuffer::_findEndPoint(const char *start, const char *&end) {
  ...
  --end;
  if (m_bJISCode) {
    for (int decrimentCount = 0; start < end && decrimentCount < kMaxDecriment;
         ++decrimentCount, --end) {
    ...
    for (int decrimentCount = 0; start < end && decrimentCount < kMaxDecriment;
      ...
      if (/*0x20*/ 0 < code &&
          code <=
              0x7E) { // acii	// Shift-JISだと2byte目の可能性もあるが大丈夫
        ++end;
        return decrimentCount;
      }
    }
...
static std::pair<int32_t, std::string> GetCharaCode(const std::string &data) {
  ...
  try {
    ...
    const UCharsetMatch **matchers =
        ucsdet_detectAll(pDectator, &founded, &err);
    if (founded > 0) {
...
void CTextBuffer::_decideCharset() {
  ...
  auto funcPageCharset = [this]() -> std::string {
    ...
    if (m_owner.fileType == "htm") {
      static std::regex rxMeta("(?:<meta ([^>]+)>|</head>)",
      ...
      while (std::regex_search(begin, m_buffer.cend(), result, rxMeta)) {
        ...
        static std::regex rx1("http-equiv=(?:\"|')?Content-Type(?:\"|')? "
                              "[^>]*charset=(?:\"|')?([^\"' >]+)(?:\"|')?",
                              std::regex_constants::icase);
        static std::regex rx2("charset=(?:\"|')?([^\"' >]+)(?:\"|')? "
                              "[^>]*http-equiv=(?:\"|')?Content-Type(?:\"|')?",
                              std::regex_constants::icase);
        static std::regex rx3("charset=(?:\"|')?([^\"' >]+)(?:\"|')?",
                              std::regex_constants::icase);
  ...
  if (kThreshold <=
      supposeCharaCode.first) { // 推定が閾値を超えていれば判定を優先する
  ...
  if (CSettings::s_pPriorityCharsetMatcher->match(urlFromHost, &filter)) {
    try {
      PriorityCharset priority = static_cast<PriorityCharset>(
          std::stoi(filter.memoryTable[0].getValue()));
      switch (priority) {
  ...
  auto found = g_mapConverter.find(charaCode);
  if (found != g_mapConverter.end()) {
    m_pConverter = found->second;
  }
...
void CTextBuffer::DataFeed(const std::string &data) {
  ...
  if (m_bCharaCodeDectated == false) {
    ...
    if (m_buffer.size() > 4) {
      // 'wOFF'
      if (m_buffer[0] == 'w' && m_buffer[1] == 'O' && m_buffer[2] == 'F' &&
          m_buffer[3] == 'F') {
        m_bBinary = true;
  ...
  if (m_bDataDump == false) { // 通常時
    ...
    if (validBufferSize > 0) {
      ...
      UnicodeString appendBuff(m_buffer.c_str(), (int32_t)validBufferSize,
                               m_pConverter, err);
      ATLASSERT(U_SUCCESS(err));
    ...
    UErrorCode err = U_ZERO_ERROR;
    UnicodeString appendBuff(m_buffer.c_str(), (int32_t)m_buffer.length(),
                             m_pConverter, err);
    ATLASSERT(U_SUCCESS(err));
    m_buffer.clear();
    m_unicodeBuffer.append(appendBuff);
  }
  const UChar *bufStart =
      reinterpret_cast<const wchar_t *>(m_unicodeBuffer.getBuffer());
  ...
  if (m_owner.bypassBody == false) { // Debug + BypassBodyの時もあるので
    ...
    for (; index <= bufEnd;
         m_currentFilter = m_vecpTextfilters.begin(), ++index) {
      for (; m_currentFilter != m_vecpTextfilters.end(); ++m_currentFilter) {
        ...
        if ((*m_currentFilter)->bypassed ||
            (index < bufEnd &&
             !(*m_currentFilter)->okayChars[(unsigned char)(*index)]))
          continue;
        ...
        if (ret < 0) {
          ...
          CLog::FilterEvent(kLogFilterTextMatch, m_owner.requestNumber,
                            UTF8fromUTF16((*m_currentFilter)->title),
                            "" /*occurrence*/);
          // CLog::FilterEvent(kLogFilterTextReplace, m_owner.requestNumber,
          // (*m_currentFilter)->title, replaceText);

          escapeOutput(out, done, CharCount(index, done));
          if (m_owner.url.getDebug()) {
            if (m_owner.url.getSource()) {
              string buf = CUtil::SimpleHtmlEscape(
                  ConvertFromUTF16(replaceText, m_pConverter));
              out << buf;
            } else {
              std::wstring occurrence(
                  index, CharCount((*m_currentFilter)->endOfMatched, index));
              string buf = "<div class=\"match\">"
                           "<div class=\"filter\">Match: ";
              buf += CUtil::SimpleHtmlEscape(
                  ConvertFromUTF16((*m_currentFilter)->title, m_pConverter));
              buf += "</div><div class=\"in\">";
              buf += CUtil::SimpleHtmlEscape(
                  ConvertFromUTF16(occurrence, m_pConverter));
          ...
          if (m_owner.killed) {
            ...
            // m_buffer.replace(0, (size_t)((*m_currentFilter)->endOfMatched -
            // bufStart),
            //               replaceText);
            // bufStart = m_buffer.data();
            m_unicodeBuffer.replace(
                0, CharCount((*m_currentFilter)->endOfMatched, bufStart),
                replaceText.c_str(), (int32_t)replaceText.length());
            len = m_unicodeBuffer.length();
            bufStart =
                reinterpret_cast<const UChar *>(m_unicodeBuffer.getBuffer());
            bufEnd = bufStart + len;
            index = bufStart;
            done = bufStart;
...
void CTextBuffer::escapeOutput(std::stringstream &out, const UChar *data,
                               size_t len) {
  if (m_owner.url.getDebug()) {
    std::wstring buf =
        CUtil::SimpleHtmlEscape(data, len); // CUtil::htmlEscape(data, len);
...
void CTextBuffer::_firstDebugOutput(const std::string &charaCode) {
  ...
  boost::algorithm::replace_first(
      content, "%%URL%%",
      CUtil::SimpleHtmlEscape(UTF8fromUTF16(m_owner.url.getUrl())));
  std::string responseLine = m_owner.responseLine.ver + " " +
                             m_owner.responseLine.code + " " +
                             m_owner.responseLine.msg;
  ...
  boost::algorithm::replace_first(content, "%%filteredInHeaders%%",
                                  filteredInHeaders);
